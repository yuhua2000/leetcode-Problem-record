...
func minimumValueSum(nums []int, andValues []int) int {
	...
	dfs = func(idx, subIdx, andValue int) int {
		key := idx*m + subIdx
		if idx >= n || subIdx >= m {
			if idx >= n && subIdx >= m && andValue == INF {
				return 0
			}
			return INF
		}
		...
		if val, ok := memo[key][andValue]; ok {
			return val
		}

		andValue &= nums[idx]
		if andValue&andValues[subIdx] < andValues[subIdx] {
			return INF
		}

		res := dfs(idx+1, subIdx, andValue)
		if andValue == andValues[subIdx] {
			res = min(res, dfs(idx+1, subIdx+1, INF)+nums[idx])
		}
		memo[key][andValue] = res
		return res
	}
...
func main() {
	minimumValueSum([]int{1, 4, 3, 3, 2}, []int{0, 3, 3, 2})
}
